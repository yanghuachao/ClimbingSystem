...
bool UCustomMovementComponent::CanClimbDownLedge()
{
	...
	const FVector WalkableSurfaceTraceStart = ComponentLocation + ComponentForword * ClimbDownWalkableSurfaceTrace;
	...
	const FVector LedgeTraceStart = WalkableSurfaceHit.TraceStart + ComponentForword * ClimbDownLedgeSurfaceOffset;
...
bool UCustomMovementComponent::CheckCanHopUp(FVector& OutHopUpTargetPosition)
{
	...
	FHitResult HopUpHit = TraceFormEyeHeight(100.f,-30.f);
	FHitResult SatfLedgeHit = TraceFormEyeHeight(100.f,150.f);
...
{
	FHitResult HopDownHit = TraceFormEyeHeight(100.f, -300.f);
